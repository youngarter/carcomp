    ...
    return (
        <div className="bg-white rounded-[3rem] p-10 shadow-[0_15px_40px_rgba(0,0,0,0.03)] border border-zinc-100">
            <div className="flex flex-col md:flex-row items-start md:items-center justify-between mb-12 gap-6">
                ...
                <button type="button" className="shrink-0 bg-zinc-900 text-white px-8 py-5 rounded-2xl font-black uppercase tracking-widest text-xs flex items-center justify-center gap-3 hover:bg-zinc-800 transition-all active:scale-95 shadow-lg shadow-zinc-900/20">
                    ...
                    <Link key={car.id} href={`/car/${car.carModel.brand.slug}/${car.carModel.slug}/${car.slug}`} className="block group">
                        <div className="bg-zinc-50 rounded-[2rem] overflow-hidden border border-zinc-100 hover:border-emerald-500/30 hover:shadow-xl hover:shadow-emerald-500/10 transition-all group-hover:-translate-y-2">
                            <div className="relative aspect-[4/3] bg-white">
                                <Image
                                    src={car.image || car.carModel.image || "/placeholder-car.jpg"}
                            ...
                            <div className="p-6">
                                ...
                                <div className="flex flex-col gap-2 mb-6">
                                    <span className="flex items-center gap-2 text-xs font-bold text-zinc-600 bg-white px-3 py-2 rounded-xl">
                                        <Fuel className="w-3 h-3 text-emerald-500" /> {car.energy || "N/A"}
                                    </span>
                                    <span className="flex items-center gap-2 text-xs font-bold text-zinc-600 bg-white px-3 py-2 rounded-xl">
                                        <Zap className="w-3 h-3 text-amber-500" /> {car.power ? `${car.power} ch` : "N/A"}
                                    </span>
                                </div>
                                ...
                                <div className="pt-4 border-t border-zinc-200/50">
                                    ...
                                    <p className="font-black text-zinc-900 text-xl">{car.startPrice?.toLocaleString('fr-FR')} <span className="text-[10px] text-zinc-400 font-bold uppercase">MAD</span></p>
                                </div>
                            </div>
                        </div>
                    </Link>
